            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" readonly value="{!! $kecamatan->nama !!}">
                    </div>
...
@section('js')
    ...
    <script>
        ...
        function del(id) {
            $.ajax({
                url: '{{ url('kelurahan/delete?id') }}=' + id,
                type: 'get',
                success: function (d) {
                    if (d == 'success') {
                        Swal.fire({
        ...
        $(document).on('click','.btn-tps', function () {
            var id = $(this).data('id')
            window.location = "{!! url('tps?kelurahan_id') !!}="+id
        })

        function getAll() {
            $.ajax({
                type: 'get',
                url: "{!! url('kelurahan/get-all?kecamatan_id=') !!}"+kecamatan_id,
                success: function (r) {
                    console.log(r)
                    var data = []
                    var no = 1;
                    $.each(r.kelurahan, function (i, d) {
                        var btn_tps = '<button class="btn btn-primary btn-xs btn-tps" data-id="'+d.id+'">'+d.tps.length+' TPS</button>'

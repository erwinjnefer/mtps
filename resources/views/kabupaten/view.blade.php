...
@section('js')
    ...
    <script>
        ...
        function del(id) {
            $.ajax({
                url: '{{ url('kabupaten/delete?id') }}=' + id,
                type: 'get',
                success: function (d) {
                    if (d == 'success') {
                        Swal.fire({
        ...
        $(document).on('click','.btn-kec', function () {
            var id = $(this).data('id')
            window.location = "{!! url('kecamatan?kabupaten_id') !!}="+id
        })

        function getAll() {
            $.ajax({
                type: 'get',
                url: "{!! url('kabupaten/get-all') !!}",
                success: function (r) {
                    var data = []
                    var no = 1;
                    $.each(r.kabupaten, function (i, d) {
                        var btn_kecamatan = '<button class="btn btn-primary btn-xs btn-kec" data-id="'+d.id+'">'+d.kecamatan.length+' Kecamatan</button>'
